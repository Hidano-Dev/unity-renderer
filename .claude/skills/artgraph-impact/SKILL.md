...
**Evidence-only reach is one hop, not a bridge (issue #361).** A REQ or file reached ONLY through observational evidence — a coverage-derived `exercises` edge, or a doc-to-doc `depends_on`/`derives_from` link with no explicit frontmatter/annotation declaration behind it (just a markdown link an author happened to write) — still shows up in `impactReqs`/`affectedFiles`/`affectedDocs`, but does not itself cascade to a further hop. Don't expect `impact` to walk PAST such a node to whatever it, in turn, incidentally touches; a `@impl`/`verifies`/frontmatter-declared edge is what keeps cascading. This narrows the reported blast radius on evidence-heavy repos compared to earlier artgraph versions — a smaller `impactReqs` here is expected precision, not a bug.
