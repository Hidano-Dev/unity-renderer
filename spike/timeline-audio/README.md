...
**扱い（2026-08-23 ユーザー判断で確定）**: 式（design ステップ 1–2）を変更する根拠は無い（構造は上記のとおり検証済みで、ffmpeg 側は厳密）。

**同期精度の判定基準は「ffmpeg 出力のクリック位置が MixPlan の計算値と一致すること」とする。Unity Editor 実再生音との比較は参考値に留め、合否判定には用いない。**

根拠: 映像は Recorder がフレーム厳密に書き出す（実測 630 フレーム / 21.000000 秒）。音声を公称タイムライン位置へ配置することがそのまま映像フレームとの厳密一致になる（mux 後のストリーム長差 0.000 フレームを実測）。Unity Editor の再生音が仮にずれていても、それは Editor 再生側の挙動であって最終成果物の同期の正解ではない。

**既知の制約**: Unity が実際にずれて再生している場合、「Editor で聞いた音」と「最終成果物の音」に最大 57 ms 程度の体感差が生じうる。

**切り分けが必要になった場合の手順**: `video-capture-start.cs` の `MovieRecorderSettings.CaptureAudio` を `true` にして映像・音声を 1 パス収録すると、Unity 自身が映像と音声を揃えたファイルができる。そのファイル内のクリック位置を `analyze-wav.ps1` で測り、公称位置と一致すれば本スパイクの音声単独収録（`AudioRecorderSettings`）が測定アーティファクトだったと確定できる。同じずれが出れば Unity が実際にずれて再生していることになる。**いずれの結果でも実装方針は変わらない**ため、必要が生じた時点で実施する。
...
- **Q-10 の絶対位置ずれの発生源切り分け**（Timeline の音声スケジューリング / Recorder の音声収録経路）は**未実施**。ただし同期精度の判定基準は「計画値との一致」で確定済み（2026-08-23 ユーザー判断）のため、実装・E2E ともにブロックしない。切り分け手順は Q-10 節に記載。

**ユーザー承認**:

- **Q-10 の判定基準**: **承認済み（2026-08-23）**。「ffmpeg 出力のクリック位置が MixPlan の計算値と一致すること」を合否基準とし、Unity 実再生音との比較は参考値に留める。
- **GO 判定**: **承認済み（2026-08-23）**。Requirement 12.2 / 12.3 の実装ゲートを通過。タスク 2 以降の実装に着手してよい。
