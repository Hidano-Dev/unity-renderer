...
// 上限は CLI 側の preflight と scene-job で検証済み。ここでは checked 変換で
// 万一の溢れを黙って 1 フレーム扱いにせず、明示的に失敗させる
var totalFrames = checked((int)System.Math.Round((outPoint - inPoint) * frameRate));
if (totalFrames < 1)
    totalFrames = 1;
